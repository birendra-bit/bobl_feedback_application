            ...
            <React.Fragment>
                    ...
                    <div className='collapse navbar-collapse align-middle' id='navbarSupportedContent'>
                        <ul className='navbar-nav ml-auto '>
                            ...
                            <li className='nav-item mt-2' style={{ paddingTop: "10px" }}>
                                <span className='text-md font-weight-bold username' onClick={() => this.showUser()}>
                                    {sessionStorage.getItem("fullName")}
                                </span>
                            </li>
                            <li className='nav-item username'>
                                {/* need to get the image from the session rather than from the state. */}
                                <img alt='' className='profile' src={"http://192.168.70.38" + this.state.userDetail.image} onClick={() => this.showUser()} />
                            </li>
                        </ul>
                    </div>
                </Navbar>
                ...
                <Modal show={this.state.show} onHide={() => this.setState({ show: false })} animation={false}>
                    <Modal.Body style={{ padding: "20px" }}>
                        <Row>
                            <Col md={5} lg={5} sm={5} xs={5}>
                                <img src={"http://192.168.70.38" + this.state.userDetail.image} alt='user' className='user_image' />
                            </Col>
                            <Col md={7} lg={7} sm={7} xs={7} style={{ marginTop: "20px" }}>
                                ...
                                <p>
                                    <span className='info'> Department: </span> {this.state.userDetail.department}
                                </p>
                                <p>
                                    <span className='info'> Branch: </span> {this.state.userDetail.branch}
                                </p>
                            </Col>
                        </Row>
                    </Modal.Body>
                    <Modal.Footer>
                        ...
                        <Button variant='secondary' onClick={() => this.setState({ show: false })} style={{ padding: "1px 7px", color: "white" }}>
                            Close
                        </Button>
                    </Modal.Footer>
                </Modal>
