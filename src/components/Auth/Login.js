...
function Login(props) {
  ...
  const handleSubmitClick = (e) => {
    ...
    axios
      .post("http://192.168.70.38/api/method/login", data,headers: {"Access-Control-Allow-Origin": "*"})
      .then(function (response) {
        var url = `http://192.168.70.38/api/method/frappe.auth.get_loggedin_user_details`,headers: {"Access-Control-Allow-Origin": "*"};
        axios
          .get(url)
          .then((resp) => {
            setState((prevState) => ({
        ...
        {state.loader ? (
          ...
          <React.Fragment>
            <Col className="my-auto" md={6} lg={3} sm={10} xs={10}>
              ...
              <div
                className="alert alert-success mt-2 login-message"
              ...
              <div
                className="alert alert-danger mt-2 login-message"
                style={{ display: state.failedMessage ? "block" : "none" }}
                role="alert"
              >
                {state.failedMessage}
              </div>
